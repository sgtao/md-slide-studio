...
- **テンプレート挿入**: エディタの「➕ テンプレート挿入」から、全17type・20テンプレートの
  正しい記法をワンクリックで挿入
- **ヘルプチートシート**: ❓ボタンから記法チートシート／キーボードショートカット／制約ルールを一覧表示
...
npm run verify   # typecheck + test + lint + e2e を一括実行
...
- `==テキスト==` はアクセント色の強調
- `fit` オプションで内容過多スライドを自動縮小、`layout:` で
...
- 全typeで共通ヘッダキーが使える: `badge:`（見出し左のピル）/ `lead:`（補足1行）/
  `point:`（下部の💡強調帯）
- `steps` type: 手順のカード型フロー。```steps ブロックに style（cards / circled）と
  items（icon / title / desc）、任意の ratio（比率帯）をYAMLで記述
- ディレクティブの `tone: dark` でそのスライドだけ地色を反転
- Studio独自拡張の仕様は [`docs/references/markdown-format-ext.md`](docs/references/markdown-format-ext.md) を参照
- `contrast` type: 「思い込み → 実際」のような対比構造を示す専用type。```contrast
  ブロックに example（比較対象の実例）と verdict（結論）をYAMLで記述
...
├── templates/       # テンプレート挿入機能のスニペット集（slideTemplates.ts）
├── help/            # ヘルプモーダルの静的コンテンツ（shortcuts・制約ルール）
