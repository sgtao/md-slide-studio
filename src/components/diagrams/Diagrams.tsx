...
const FLOW_TABLE: Record<number, { width: number; xs: number[]; arrows: [number, number][] }> = {
  ...
  3: { width: 267, xs: [40, 347, 653], arrows: [[307, 347], [614, 654]] },
  4: { width: 190, xs: [40, 270, 500, 730], arrows: [[230, 270], [460, 500], [690, 730]] },
  5: { width: 144, xs: [40, 224, 408, 592, 776], arrows: [[184, 224], [368, 408], [552, 592], [736, 776]] },
};
...
function NodeLabel({ x, y, text, className = 'diagram-label' }: {
  x: number; y: number; text: string; className?: string;
}) {
  if (text.length <= 8) {
    return <text className={className} x={x} y={y}>{text}</text>;
  }
  const mid = Math.ceil(text.length / 2);
  return (
    <text className={className} x={x} y={y}>
      <tspan x={x} dy="-0.6em">{text.slice(0, mid)}</tspan>
      <tspan x={x} dy="1.2em">{text.slice(mid)}</tspan>
    </text>
  );
}

function DiagramFrame({ markerId, title, source, children }: {
  markerId: string; title: string; source?: string; children: React.ReactNode;
}) {
  return (
    <svg viewBox="0 0 960 480" xmlns="http://www.w3.org/2000/svg" className="diagram-svg" role="img">
      <title>{title}</title>
      <defs>
      ...
      {children}
      {source && (
        <text x={920} y={470} className="diagram-source" textAnchor="end">出典: {source}</text>
      )}
    </svg>
  );
}

export function FlowDiagramSvg({ diagram, slideIndex, source }: {
  diagram: DiagramBlock; slideIndex: number; source?: string;
}) {
      ...
      {nodes.map((label, i) => {
        ...
        return (
          <Fragment key={i}>
            ...
            {diagram.labels?.[i] ? (
              <text className="diagram-sublabel" x={cx} y={300}>{diagram.labels[i]}</text>
            ) : null}
          </Fragment>
        );
      })}
      {t.arrows.slice(0, nodes.length - 1).map(([x1, x2], i) => (
        <path key={`e${i}`} className="diagram-edge" markerEnd={`url(#${markerId})`} d={`M${x1},230 L${x2},230`} />
      ))}
    </DiagramFrame>
  );
}

export function LayerDiagramSvg({ diagram, slideIndex, source }: {
  diagram: DiagramBlock; slideIndex: number; source?: string;
}) {
        ...
        return (
          <Fragment key={li}>
            {boxes.slice(0, k).map((label, bi) => (
              <Fragment key={bi}>
                <rect className={li === L - 1 ? 'diagram-node diagram-node-accent' : 'diagram-node'}
                  x={lx.xs[bi]} y={y} width={lx.width} height={ly.h} rx={8} />
...
export function CycleDiagramSvg({ diagram, slideIndex, source, centerLabel }: {
  diagram: DiagramBlock; slideIndex: number; source?: string; centerLabel?: string;
}) {
  ...
  const markerId = `arrow-s${slideIndex}`;
  return (
    <DiagramFrame markerId={markerId} title={`サイクル図: ${nodes.join(' → ')} → …`} source={source}>
      {centerLabel && <NodeLabel x={480} y={240} text={centerLabel} className="diagram-label diagram-center" />}
      {nodes.map((label, i) => (
        <Fragment key={i}>
      ...
      {arrows.map(([x1, y1, x2, y2], i) => (
        <path key={`e${i}`} className="diagram-edge" markerEnd={`url(#${markerId})`} d={`M${x1},${y1} L${x2},${y2}`} />
      ))}
    </DiagramFrame>
  );
}
